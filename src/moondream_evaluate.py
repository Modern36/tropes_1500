from trope_paths import read_data, moon_tmp_dir, output_dir
...
for key, outcomes in outcome_dict.items():
    ...
    p_p.append(pred["person"] > 0)
    m_p.append(pred["man"] > 0)
    w_p.append(pred["woman"] > 0)

report_p = classification_report(p_gt, p_p)
report_m = classification_report(m_gt, m_p)
report_w = classification_report(w_gt, w_p)

with open(output_dir / "moondream_report.md", "w", encoding="utf-8") as f:
    f.write(
